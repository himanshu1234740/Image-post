...
async function register() {
  ...
  if (reg.test(password)) {
    ...
    swal(
      "Oops!",
      "Password Should Be Minimum 8 characters And Maximum 15 characters",
      "error"
    );
  }
}
...
function update_profile(id) {
  ...
  if (regex.test(email)) {
    axios
      .put(`api/login/${id}`, { user, email })
      .then(function (response) {
        if (response.data === "invalid") {
          swal(
            "Oops!",
            "This Email is Already Exists You Can't Update'!",
            "error"
          );
        } else {
...
function updatePassword() {
  ...
  if (reg.test(password)) {
    axios
      .put(`/api/forget/${id}`, { password, cpassword })
      .then((data) => {
